  ...
  list.reverse().forEach(function(item, index) {
    ...
    if (item.type === 'link_request') {
      ...
      html += [
        ...
        '<span style="color:#888;">👤 家长：</span><span style="color:#5d4037; font-weight:bold;">' + item.parentName + '</span>',
        '<span style="color:#888;">📧 邮箱：</span><span>' + item.parentEmail + '</span>',
        '<span style="color:#888;">👶 学生：</span><span style="font-weight:bold;">' + item.studentName + '</span>',
      ...
      html += [
        ...
        '<div style="display:flex; justify-content:space-between; align-items:start; flex-wrap:wrap; gap:8px;">',
        '<h4 style="color:#5d4037; margin:0;">👤 ' + item.parentName + unreadDot2 + '</h4>',
        '<span style="color:#999; font-size:0.8em;">' + item.time + '</span>',
        '</div>',
        '<div style="margin-top:12px; display:grid; grid-template-columns:auto 1fr; gap:6px 12px; font-size:0.9em;">',
